...
def get_topic_resource(topic_resource_id: int, db: Session = Depends(get_db)):
...
def create_topic_resource(payload: TopicResourceCreate, db: Session = Depends(get_db)):
...
def delete_topic_resource(topic_resource_id: int, db: Session = Depends(get_db)):
    tr = db.query(TopicResource).filter(TopicResource.id == topic_resource_id).first()
    if not tr:
        raise HTTPException(status_code=404, detail="TopicResource not found")
    db.delete(tr)
