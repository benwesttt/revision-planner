  ...
  return (
    <span className="flex gap-0.5">
      {[1, 2, 3, 4, 5].map(n => (
        <svg
          key={n}
          className={`w-3 h-3 ${n <= value ? 'text-accent' : 'text-ink-muted'}`}
    ...
    <div className="max-w-3xl">
      ...
      {!hasPlan && (
        <div className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <p className="text-ink-secondary text-sm font-medium">No plan generated yet</p>
      ...
      {hasPlan && (
        <>
          ...
          {todayBlocks.length === 0 ? (
            <p className="text-sm text-ink-muted mb-8">No sessions scheduled for today.</p>
          ) : (
                    ...
                    {!loggingBlockId || loggingBlockId !== block.id ? (
                      <div className="flex justify-end mt-1">
                        {loggedBlockIds.has(block.id) ? (
                          <span className="text-xs font-medium px-3 py-1.5 rounded-lg bg-success-bg text-success border border-success/40">
                            ✓ Logged
                          </span>
                        ) : (
                          <button
                            onClick={() => { setLoggingBlockId(block.id); setLogConfidence(3); }}
                            className="text-xs px-4 py-1.5 rounded-lg bg-accent hover:bg-accent-hover text-background font-medium transition-colors"
                          >
                            Log
                          </button>
                        )}
                      ...
                      <div className="mt-2 pt-2 border-t border-border flex items-center gap-3 flex-wrap">
                        ...
                        <div className="flex gap-1">
                          {[1, 2, 3, 4, 5].map(n => (
                            ...
                            >
                              <svg
                                className={`w-5 h-5 transition-colors ${n <= logConfidence ? 'text-accent' : 'text-ink-muted hover:text-ink-secondary'}`}
                                viewBox="0 0 20 20" fill="currentColor"
                              >
                                <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                              </svg>
                        ...
                        <button
                          ...
                          className="px-3 py-1 bg-accent hover:bg-accent-hover text-background text-xs font-medium rounded-lg transition-colors"
                        >
                          Confirm
                        </button>
                        <button
                      ...
                      <div className="flex items-center gap-1.5 mb-0.5">
                        ...
                        {course && (
                          <>
                            ...
                            <span className="text-sm text-ink-secondary truncate">{course.name}</span>
                          </>
                        )}
                      </div>
            ...
            {neglectedTopics.map(t => {
              ...
              return (
                ...
                >
                  ...
                  <span className={`text-xs font-medium shrink-0 ${daysAgoVal === null ? 'text-warning' : 'text-ink-secondary'}`}>
                    {lastRevisedLabel}
                  </span>
                </div>
              );
                ...
                >
                  ...
                  <div className="flex flex-col gap-1">
                    {s.behind_pace.map(t => {
                      ...
                      const labelCls = days !== null && days >= -7 ? 'text-warning' : 'text-danger';
        ...
        <div className="mt-8">
          ...
          <div className="flex flex-col gap-2">
            {upcomingAssessments.map(a => {
              ...
              const borderCls = days < 0 ? 'border-danger' : days <= 7 ? 'border-danger' : days <= 14 ? 'border-warning' : 'border-success';
              const labelCls = days < 0 ? 'text-danger' : days <= 7 ? 'text-danger' : days <= 14 ? 'text-warning' : 'text-success';
              ...
              return (
                <div
                  key={a.id}
                  className={`bg-surface border ${borderCls} rounded-xl px-4 py-3 flex items-center justify-between gap-4`}
                >
                  <div className="min-w-0">
