  ...
  return (
    <div className="flex gap-1.5">
      {[1, 2, 3, 4, 5].map(n => (
        ...
        >
          <svg
            className={`w-7 h-7 transition-colors ${n <= value ? 'text-accent' : 'text-ink-muted hover:text-ink-secondary'}`}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 px-4">
      <div className="bg-surface border border-border rounded-xl p-5 w-full max-w-sm">
        <h2 className="text-lg font-semibold text-ink mb-1">Finish Session</h2>
        {topicName && <p className="text-sm text-ink-secondary mb-4">{topicName}</p>}

        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          {isLong && (
            <p className="text-xs text-warning bg-warning-bg border border-warning/40 rounded-lg px-3 py-2">
